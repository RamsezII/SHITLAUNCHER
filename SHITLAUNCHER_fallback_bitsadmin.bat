...
:: === Chemin du curl local ===
set "CURL_PATH=%~dp0curl.exe"

:: === Fonction : telecharger curl si absent ===
:check_curl
if exist "!CURL_PATH!" (
    echo [OK] curl.exe trouve localement.
    goto :main
)

echo [INFO] curl.exe non present. Tentative de telechargement...

:: === Essaye bitsadmin ===
where bitsadmin >nul 2>nul
if %errorlevel% equ 0 (
    bitsadmin /transfer "curlDownload" ^
    https://curl.se/windows/dl-8.6.0_2/curl-8.6.0_2-win64-mingw.zip ^
    "%TEMP%\\curl.zip"
) else (
    echo [INFO] bitsadmin non disponible, tentative via PowerShell...
    powershell -Command "Invoke-WebRequest 'https://curl.se/windows/dl-8.6.0_2/curl-8.6.0_2-win64-mingw.zip' -OutFile '%TEMP%\\curl.zip'"
)

:: === Vérifie si le zip a été téléchargé ===
if not exist "%TEMP%\\curl.zip" (
    echo [ERREUR] Impossible de telecharger curl.
    pause
    exit /b 1
)

:: === Extraction via PowerShell ===
echo [INFO] Extraction de curl...
powershell -Command "Expand-Archive '%TEMP%\\curl.zip' -DestinationPath '%TEMP%\\curl-unzip' -Force"

:: === Copie curl.exe ===
if exist "%TEMP%\\curl-unzip\\curl-8.6.0_2-win64-mingw\\bin\\curl.exe" (
    copy "%TEMP%\\curl-unzip\\curl-8.6.0_2-win64-mingw\\bin\\curl.exe" "%~dp0" >nul
    echo [OK] curl.exe installe localement.
) else (
    echo [ERREUR] Impossible de trouver curl.exe dans l'archive.
    pause
    exit /b 1
)
